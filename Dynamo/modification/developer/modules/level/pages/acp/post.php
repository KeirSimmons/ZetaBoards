	...
	class page implements iPage {
		public function __construct() {
			...
			if($this -> db -> groups["id-" . $this -> db -> my["gid"]]["admin"] == 0) {
				...
				$this -> db -> notice_id = "level-acp-post";
			}
		}
		...
		private function groups() {
			...
			$this -> db -> cb = "dynamo.level.acp.post.groups";
		}
		...
		private function form() {
			if(isset($this -> info["gid"])) {
				...
				if(isset($this -> db -> groups["id-" . $gid])) {
					...
					$this -> db -> cb = "dynamo.level.acp.post.form";
				}
			} else {
				$this -> groups();
			}
		...
		private function edit() {
			...
			if(
				...
				$to_update = array();
				$to_update[] = array("level", "reply_min", $gid, min($reply_min, $reply_max));
				$to_update[] = array("level", "reply_max", $gid, max($reply_min, $reply_max));
				$to_update[] = array("level", "topic_min", $gid, min($topic_min, $topic_max));
				$to_update[] = array("level", "topic_max", $gid, max($topic_min, $topic_max));
